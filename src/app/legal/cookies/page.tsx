...
export const metadata: Metadata = { title: "Cookie Policy | IFinCert" };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-ink-900">
      <div className="mb-12 border-b border-sand-200 pb-6">
        ...
        <h1 className="font-display text-4xl font-bold text-ink-950 mb-3">Cookie Policy</h1>
      ...
      <Section title="2. How IFinCert Uses Cookies">
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b-2 border-sand-200">
                <th className="text-left py-3 pr-4 text-ink-200 font-semibold">Type</th>
                <th className="text-left py-3 pr-4 text-ink-200 font-semibold">Purpose</th>
                <th className="text-left py-3 pr-4 text-ink-200 font-semibold">Examples</th>
                <th className="text-left py-3 text-ink-200 font-semibold">Opt-out</th>
              </tr>
            </thead>
            <tbody>
              {cookieTypes.map((c) => (
                <tr key={c.name} className="border-b border-ink-900">
                  <td className="py-3 pr-4 font-medium text-ink-950">{c.name}</td>
                  <td className="py-3 pr-4 text-ink-400">{c.purpose}</td>
                  <td className="py-3 pr-4 text-ink-400">{c.examples}</td>
                  <td className="py-3">
                    {c.canOptOut ? (
